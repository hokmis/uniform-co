-- Stocktake POST uses balance version fencing; stale counts must be re-counted.

create table public.stocktakes (
  id uuid primary key default gen_random_uuid(),
  stocktake_no text not null unique check (btrim(stocktake_no) <> ''),
  warehouse_id uuid not null references public.warehouses(id),
  status text not null default 'DRAFT' check (status in ('DRAFT', 'STALE_COUNT', 'POSTED')),
...
create table public.stocktake_lines (
  ...
  book_quantity_snapshot bigint not null check (book_quantity_snapshot >= 0),
  balance_version_snapshot bigint not null check (balance_version_snapshot >= 0),
  book_captured_at timestamptz not null default now(),
  counted_quantity bigint not null check (counted_quantity >= 0),
  counted_at timestamptz not null default now(),
  difference_quantity bigint generated always as (
    counted_quantity - book_quantity_snapshot
  ) stored,
  reason text,
  unique (stocktake_id, item_id),
  check (difference_quantity = 0 or btrim(coalesce(reason, '')) <> '')
);

create or replace function private.prevent_posted_stocktake_mutation()
returns trigger
language plpgsql
security definer
...
begin
  if tg_table_name = 'stocktakes' then
    if old.status = 'POSTED' then
      raise exception 'Posted stocktakes are immutable';
    end if;
    if old.warehouse_id <> new.warehouse_id then
      raise exception 'A stocktake cannot move to another warehouse';
    end if;
  elsif exists (
    select 1 from public.stocktakes s
    where s.id = old.stocktake_id and s.status = 'POSTED'
  ) then
    raise exception 'Posted stocktake lines are immutable';
  end if;
  return new;
end;
$$;

create trigger stocktakes_immutable_guard
before update or delete on public.stocktakes
for each row execute function private.prevent_posted_stocktake_mutation();
create trigger stocktake_lines_immutable_guard
...
create policy stocktake_lines_read on public.stocktake_lines
  for select to authenticated using (private.has_role('HR') or private.has_role('WAREHOUSE'));
create policy stocktakes_draft_insert on public.stocktakes
  for insert to authenticated with check (
    created_by = private.current_account_id()
    and status = 'DRAFT'
    and exists (
      select 1 from public.warehouses w
      where w.id = warehouse_id
        and ((w.purpose = 'HR' and private.has_role('HR'))
          or (w.purpose = 'GENERAL' and private.has_role('WAREHOUSE')))
    )
  );
create policy stocktakes_draft_update on public.stocktakes
  for update to authenticated using (
    created_by = private.current_account_id() and status = 'DRAFT'
  ) with check (
    created_by = private.current_account_id() and status = 'DRAFT'
  );
create policy stocktake_lines_draft_insert on public.stocktake_lines
  for insert to authenticated with check (exists (
    select 1 from public.stocktakes s
    where s.id = stocktake_id and s.created_by = private.current_account_id()
      and s.status = 'DRAFT'
      and ((private.has_role('HR') and exists (select 1 from public.warehouses w where w.id = s.warehouse_id and w.purpose = 'HR'))
        or (private.has_role('WAREHOUSE') and exists (select 1 from public.warehouses w where w.id = s.warehouse_id and w.purpose = 'GENERAL')))
  ));
create policy stocktake_lines_draft_update on public.stocktake_lines
  for update to authenticated using (exists (
    select 1 from public.stocktakes s
    where s.id = stocktake_id and s.created_by = private.current_account_id() and s.status = 'DRAFT'
  )) with check (exists (
    select 1 from public.stocktakes s
    where s.id = stocktake_id and s.created_by = private.current_account_id() and s.status = 'DRAFT'
  ));
revoke all on table public.stocktakes, public.stocktake_lines from public, anon, authenticated;
grant select on public.stocktakes, public.stocktake_lines to authenticated;
grant insert, update on public.stocktakes, public.stocktake_lines to authenticated;
