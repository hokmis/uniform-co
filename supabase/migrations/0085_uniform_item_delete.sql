...
-- Separate hard-delete path for uniform_items. The normal master-data RPC keeps
-- the existing stop/deactivate behavior; this RPC is intentionally stricter.
-- A product with any inventory balance or ledger row can never be hard-deleted.
...
begin
  ...
  -- Keep an immutable snapshot because the business row is about to disappear.
