-- The upload owner and the worker are intentionally different actors.  Keep
-- the owner in the immutable batch/audit fields, while authenticating the
-- execution through the bound worker identity.
