-- Allow human login names containing 2-50 lowercase ASCII letters and digits.
...
alter table public.app_accounts
  ...
  check (login_name is null or login_name ~ '^[a-z0-9]{2,50}$');

comment on column public.app_accounts.login_name is
  'Case-normalized human login name containing 2-50 lowercase ASCII letters or digits. NULL only identifies a legacy email-login account until SYSTEM_ADMIN assigns a login name.';
...
begin
  ...
  if p_auth_user_id is null
     or normalized_login_name !~ '^[a-z0-9]{2,50}$'
