...
begin
  ...
  if btrim(coalesce(p_stocktake_no, '')) = ''
    ...
    raise exception 'Stocktake draft fields are invalid';
  end if;
  line_count := jsonb_array_length(p_lines);
  if line_count = 0 or line_count > 1000 or pg_column_size(p_lines) > 10000000 then
    raise exception 'Stocktake draft line limits exceeded';
  end if;
  ...
  if exists (
    select 1
    from (
      select (value ->> 'itemId')::uuid as item_id
  ...
  insert into public.operation_commands (
    ...
    'CREATE_STOCKTAKE_DRAFT', p_idempotency_key, p_request_fingerprint, current_account
  ) on conflict (operation_code, idempotency_key) do nothing
  returning * into command_row;
  if command_row.id is null then
    select * into command_row
    from public.operation_commands
    where operation_code = 'CREATE_STOCKTAKE_DRAFT'
    ...
    raise exception using errcode = '40001', message = 'Stocktake draft is already in progress or failed';
  end if;
  ...
  for update;
  if warehouse_row.id is null then raise exception 'Active stocktake warehouse does not exist'; end if;
  if (warehouse_row.purpose = 'HR' and not private.has_role('HR'))
    ...
    raise exception using errcode = '42501', message = 'The account cannot create this warehouse stocktake';
  end if;

  select coalesce(array_agg(item_id order by item_id), '{}'::uuid[])
    into item_ids
  from (
    select distinct (value ->> 'itemId')::uuid as item_id
    from jsonb_array_elements(p_lines)
  ) requested;
  ...
  insert into public.stocktakes (stocktake_no, warehouse_id, created_by, note, counted_at)
  values (left(btrim(p_stocktake_no), 80), p_warehouse_id, current_account,
          left(nullif(btrim(coalesce(p_note, '')), ''), 2000), now())
  ...
  for line_value in select value from jsonb_array_elements(p_lines) loop
    ...
    values (p_warehouse_id, item_row.id)
    on conflict (warehouse_id, item_id) do nothing;
    perform 1 from public.inventory_balances b
    where b.warehouse_id = p_warehouse_id and b.item_id = item_row.id
    for update;
    ...
    insert into public.stocktake_lines (
      ...
      stocktake_row.id, item_row.id, book_quantity, balance_version,
  ...
  set status = 'SUCCEEDED', result_entity_type = 'stocktakes',
      result_entity_id = stocktake_row.id, succeeded_at = now()
