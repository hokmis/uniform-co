-- 0082_uniform_item_classification_tags.sql
-- Add classification tags (gender, style) to uniform_items
-- and update apply_master_import and export_master_data to handle them.

begin;

alter table public.uniform_items add column if not exists gender text;
alter table public.uniform_items add column if not exists style text;
...
begin
  ...
  if not (private.has_role('HR') or (private.has_role('PROCUREMENT') and entity_type in ('SUPPLIERS', 'SUPPLIER_ITEMS'))) then
    raise exception using errcode = '42501', message = 'Role cannot import this master data';
  end if;
  ...
  insert into public.master_import_batches (batch_no, entity_type, source_filename, created_by)
  values ('MASTER-' || substring(gen_random_uuid()::text from 1 for 12), entity_type, left(nullif(btrim(p_source_filename), ''), 255), current_account)
  ...
  for row_value in select value from jsonb_array_elements(p_rows) loop
    ...
    if error_code is null and (
      ...
      btrim(coalesce(row_value ->> 'gender', '')) ~ '^[=+@-]' or
      btrim(coalesce(row_value ->> 'style', '')) ~ '^[=+@-]' or
  ...
  for row_value in select raw_values from public.master_import_rows where batch_id = batch_row.id order by row_number loop
    if entity_type = 'INSTITUTIONS' then
      ...
      if target_id is null then
        raise exception using errcode = '40002', message = '找不到對應的機構 (' || btrim(coalesce(row_value ->> 'institutionCode', '')) || ') 的識別碼';
      end if;
      ...
      insert into public.master_import_changes (batch_id, actor_account_id, entity_type, entity_key, action, old_values, new_values)
      values (batch_row.id, current_account, entity_type, change_key, case when old_values is null then 'INSERT' else 'UPDATE' end, old_values, row_value);
      insert into public.uniform_items as target (item_code, item_name, unit, size, category, season, gender, style, is_active)
      values (
        change_key,
        btrim(row_value ->> 'name'),
        btrim(row_value ->> 'unit'),
        nullif(btrim(row_value ->> 'size'), ''),
        nullif(btrim(row_value ->> 'category'), ''),
        nullif(btrim(row_value ->> 'season'), ''),
        nullif(btrim(row_value ->> 'gender'), ''),
        nullif(btrim(row_value ->> 'style'), ''),
        coalesce((row_value ->> 'isActive')::boolean, true)
      )
      on conflict (item_code) do update set
        ...
        gender = case when row_value ? 'gender' and btrim(coalesce(row_value ->> 'gender', '')) <> '' then excluded.gender else target.gender end,
        style = case when row_value ? 'style' and btrim(coalesce(row_value ->> 'style', '')) <> '' then excluded.style else target.style end,
