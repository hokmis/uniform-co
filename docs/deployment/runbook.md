...
`0015_draft_creation_rpc.sql` 也提供 `create_replenishment_draft`；人資可從額外補庫工作台建立不預留庫存的補庫單，再由 `submit_replenishment_request` 送出。`0096_replenishment_post_ui.sql` 將倉庫前台串接到 `post_replenishment_request_with_lines`：瀏覽器只傳實際調庫量／短發原因，資料庫在同一交易內重新鎖定總倉餘額、寫入兩倉流水並將補庫單改為 `SHIPPED`；`0097_replenishment_post_lock_order.sql` 再把公開入口的品號互斥鎖提前，與送出／取消／人資發貨共用鎖序；不得以瀏覽器直接 UPDATE 已送出的補庫明細。`0016_employee_import_guard.sql` 將員工 CSV 匯入改由 `apply_employee_import_checked` 原子套用，資料庫端也會重驗檔案大小、列數、欄數與儲存格長度，重試沿用相同冪等鍵。
...
`0045_import_worker_snapshot_and_storage.sql` 提供 `list_import_work`、`get_import_reference_snapshot` 與 import object read capability。Edge Function 與 Node worker 都先用 `src/domain/import-worker-parser.ts` 驗證並解析檔案，再以 `src/domain/import-worker.ts` 依員工工號、制服品號、供應商條件及倉庫／品號鍵產生 INSERT／UPDATE／SKIP／ERROR preview，逐一完成 PARSE／VALIDATE chunk；使用者確認後才取得 batch lease 並呼叫 APPLY。Edge Function 先以已驗證使用者 JWT 讀取其 exact batch object，並在 function 內再次核對 bucket／batch prefix／bytes／SHA-256；它不會將 key、service-role 或檔案內容回傳瀏覽器。部署 smoke 必須涵蓋一列更新、一列略過、一列錯誤、逾期 chunk 接手、max-attempt FAILED、response-loss 後同一 batch／object key 重試。
...
**目前 production 特別注意（2026-09-20 查核）：** 連線中的正式 project migration ledger 只記錄到 `0067`，但 catalog 顯示後續 migration 有部分已手動套用、部分物件仍缺失。最新版唯讀 verifier 顯示修復前置 relations `47/47`、base functions `28/28` 存在，但 ready views `0/19`、indexes `0/24`、callable RPCs `0/13`，且 6 個冗餘唯一索引仍在。這個 project 暫時不得使用本節的一般 `db push`／GitHub migrations workflow；先依 [`supabase/manual/README.md`](../../supabase/manual/README.md) 的「正式工作台讀取路徑修復」執行唯讀 verifier、於 staging 驗證修復 SQL，再完成 `0068`–`0132` 實際 schema 對帳與 ledger reconciliation。針對性 SQL 不會更新 migration ledger，也不代表其他 release gate 已通過。
