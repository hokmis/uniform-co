...
describe("System Guide access contract", () => {
  ...
  it("uses the self-readable role table and cached prefetch instead of a delayed HEAD probe", () => {
    ...
    expect(shell).toContain('router.prefetch("/system-guide")');
  });
  ...
  it("uses the protected API when the self-readable role query is empty", () => {
    ...
    expect(roleBranch).toContain("const fallbackDocuments = await (prefetchedDocuments ?? requestDocuments(activeClient, activeUserId));");
  });
