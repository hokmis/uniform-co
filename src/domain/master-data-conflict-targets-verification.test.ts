...
const cleanupSql = readFileSync(
  join(process.cwd(), "supabase", "migrations", "0122_remove_redundant_master_conflict_indexes.sql"),
  "utf8",
);
...
describe("manual master-data conflict target verification SQL", () => {
  ...
  it("removes duplicate conflict indexes only after named constraints and RPC targets are verified", () => {
    const normalizedCleanupSql = cleanupSql.replace(/--[^\n]*/g, "").replace(/\s+/g, " ");
    expect(normalizedCleanupSql).toContain("pg_constraint");
    expect(normalizedCleanupSql).toContain("pg_get_functiondef");
    expect(normalizedCleanupSql).toContain("on conflict on constraint");
    expect(normalizedCleanupSql).toContain("drop index if exists public.uniform_items_item_code_conflict_idx");
    expect(normalizedCleanupSql).toContain("drop index if exists public.operation_commands_operation_idempotency_conflict_idx");
    expect(normalizedCleanupSql).toContain("errcode = '55000'");
  });
});
