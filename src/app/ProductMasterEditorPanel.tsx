...
export default function ProductMasterEditorPanel({
  ...
  async function saveForm(formToSave = form) {
    ...
    if (error) {
      ...
      if (error.message.includes("there is no unique or exclusion constraint matching the ON CONFLICT specification")) {
        ...
        setMessage(
          `保存失敗：資料庫 RPC 回報 ON CONFLICT 解析錯誤（${errorContext || "42P10"}）。若 verify-master-data-conflict-targets.sql 已確認索引存在，請在相同 Supabase project 執行 supabase/manual/0084_master_data_constraint_targets.sql，完成後重新登入再試。原始訊息：${error.message}`,
        );
        return false;
      }
  ...
  async function deleteItem() {
    ...
    if (error) {
      ...
      setMessage(`刪除失敗或結果未知：${error.message}；請先重新整理商品清單再確認結果`);
      return;
    }
  ...
  return (
    <section className="panel" id="product-master-editor" aria-label="商品主檔新增修改停用刪除">
