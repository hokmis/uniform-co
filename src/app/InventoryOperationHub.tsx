  ...
  return (
    <section className="panel" aria-label="庫存管理操作入口">
      <div className="panel-heading">
        ...
        <span className="status-pill">異動／稽核</span>
      </div>
      <p className="auth-message">庫存管理不直接編輯餘額。每個新增、修改、取消或更正都必須留在對應的草稿／完成流程，避免把帳面數字與不可變流水拆開。</p>
      <div className="summary-list">
        ...
        <div className="summary-row"><span><strong>發貨與交付</strong><small>建立理貨草稿、修改實際調庫量，再確認完成發放與扣帳。</small></span><button className="secondary-button" type="button" onClick={() => onNavigate("warehouse", "warehouse-control-title")}>建立發貨草稿</button></div>
        ...
        <div className="summary-row"><span><strong>採購入庫</strong><small>合格量確認完成後才增加總倉；未完成草稿可修改，已完成資料使用更正。</small></span><button className="secondary-button" type="button" onClick={() => onNavigate("procurement", "procurement-receipt-title")}>前往採購入庫</button></div>
      </div>
    </section>
  );
}
