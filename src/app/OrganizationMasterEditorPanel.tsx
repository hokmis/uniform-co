  ...
  return (
    <section className="panel organization-editor-panel" aria-label={`${entityLabel}新增修改停用`}>
      ...
      <div className="form-grid">
        {entityType === "DEPARTMENTS" ? <label className="field"><span>所屬機構</span><select value={form.institutionCode} onChange={(event) => updateField("institutionCode", event.target.value.trim())} disabled={busy || keyLocked || confirmationOnly}><option value="">請選擇機構</option>{institutions.filter((institution) => institution.is_active || institution.code.trim() === form.institutionCode.trim()).map((institution) => <option key={institution.id} value={institution.code.trim()}>{institution.code.trim()}｜{institution.name}{institution.is_active ? "" : "（停用）"}</option>)}</select></label> : null}
