  ...
  return <section className="panel hr-request-history-panel" aria-label="人資需求單查詢">
    ...
    {selected ? <div className="panel hr-request-history-detail" aria-live="polite">
      ...
      {detailLoading ? <p className="muted">明細讀取中…</p> : detailLoadedForRequestId !== selectedId ? <p className="auth-message">目前需求的明細尚未載入，請重新整理後再試。</p> : <>
        <h4>品號彙總</h4>
        <div className="summary-list">{items.map((item) => <div className="summary-row" key={item.id}><span><strong>{item.item_code_snapshot ?? item.item_id}</strong><small>{item.item_name_snapshot ?? "制服品號"}／{item.unit_snapshot ?? "—"}</small></span><span>F {numberValue(item.issue_quantity)} ＋ I {numberValue(item.increase_quantity)}</span><strong>R {numberValue(item.requested_transfer_quantity)}</strong></div>)}</div>
        <h4>員工發放明細</h4>
        <div className="summary-list">{issueLines.map((line) => <div className="summary-row" key={line.id}><span><strong>{line.employee_no_snapshot ?? "—"}｜{line.employee_name_snapshot ?? "—"}</strong><small>{line.institution_code_snapshot ?? "—"}／{line.department_code_snapshot ?? "—"}／{line.item_code_snapshot ?? "—"} {line.item_name_snapshot ?? ""} {line.size_snapshot ?? ""}</small></span><strong>{numberValue(line.quantity)} {line.unit_snapshot ?? ""}</strong></div>)}</div>
