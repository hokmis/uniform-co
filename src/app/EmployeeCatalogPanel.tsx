...
import { invalidateEmployeeDirectory, loadEmployeeDirectory, type EmployeeDirectoryRow } from "@/src/lib/employee-directory-read";
...
const EMPTY_EMPLOYEE_ROWS: EmployeeCatalogEntry[] = [];
...
function buildRows(employees: EmployeeDirectoryRow[]): EmployeeCatalogEntry[] {
  return employees.map((employee) => {
    return {
      id: employee.id,
      employeeNo: employee.employee_no,
      name: employee.name,
      institutionCode: employee.institution_code ?? employee.institution_id,
      institutionName: employee.institution_name ?? "機構不可讀取",
      departmentCode: employee.department_code ?? employee.department_id,
      departmentName: employee.department_name ?? "部門不可讀取",
...
export default function EmployeeCatalogPanel({ refreshToken = 0, onEdit, onDeactivate }: Props) {
  ...
  const readEmployees = useCallback(async (): Promise<AccountScopedReadOutcome<EmployeeCatalogEntry[]>> => {
    if (!client) {
      return { status: "failure", errors: [], message: "員工主檔尚未連線。請登入後重試。" };
    }
    const directoryResult = await loadEmployeeDirectory(client);
    if (directoryResult.errors.length > 0) {
      return {
        status: "failure",
        errors: directoryResult.errors,
        message: `員工主檔載入失敗：${safeSupabaseReadErrorMessage(directoryResult.errors[0])}`,
      };
    }
    const rows = buildRows(directoryResult.employees);
    exportOperation.current = null;
    return {
      status: "success",
      data: rows,
      message: `已載入 ${rows.length} 位員工；停用資料是否可見由目前角色與資料權限決定`,
    };
  }, [client]);
  const {
    data: visibleRows,
    ...
    emptyData: EMPTY_EMPLOYEE_ROWS,
    resourceLabel: "員工主檔",
    initialMessage: client ? "正在確認工作區身份…" : "預覽模式：登入後才能讀取目前帳號可見的員工主檔",
    read: readEmployees,
  });

  const displayMessage = identityError ?? message;

  const filteredRows = useMemo(() => filterEmployeeCatalog(visibleRows, { query, status, institutionCode }), [institutionCode, query, status, visibleRows]);
  const sortedRows = useMemo(() => sortEmployeeCatalog(filteredRows, sortKey, sortDirection), [filteredRows, sortDirection, sortKey]);
  const institutionOptions = [...new Map(visibleRows.map((row) => [row.institutionCode, row.institutionName])).entries()].sort(([left], [right]) => left.localeCompare(right, "zh-Hant", { numeric: true }));
  ...
  async function exportEmployees() {
    ...
    if (!hasCurrentDataSnapshot) {
      setMessage("員工主檔尚未完成載入，請稍候再匯出。");
      return;
    }
    setBusy(true);
    ...
    if (auditResult.error) {
      setBusy(false);
      setMessage(`匯出稽核失敗，未產生檔案：${safeSupabaseMutationErrorMessage(auditResult.error, "請確認已執行 0079 SQL 後重試。")}`);
      return;
    }
  ...
  return (
    <section className="panel employee-catalog-panel" aria-label="員工主檔清單" aria-busy={dataLoading || busy}>
      <div className="panel-heading">
        ...
        <div className="employee-catalog-actions"><button className="secondary-button" type="button" onClick={() => void exportEmployees()} disabled={busy || !hasCurrentDataSnapshot || (dataLoading && visibleRows.length === 0)}>{busy ? "匯出中…" : "匯出篩選結果"}</button><button className="secondary-button" type="button" onClick={() => { if (client) invalidateEmployeeDirectory(client); reload(); }} disabled={busy}>{dataLoading ? "讀取中…" : "重新整理"}</button></div>
      </div>
      {dataLoading ? <p className="sr-only" role="status" aria-live="polite">正在載入員工主檔…</p> : null}
      <div className="management-catalog-metrics" aria-label="員工主檔摘要">
        <div className="metric"><span>全部員工</span><strong>{visibleRows.length}</strong><small>目前角色可讀取資料</small></div>
        <div className="metric"><span>在職</span><strong>{visibleRows.filter((row) => row.employmentStatus === "ACTIVE").length}</strong><small>可供新需求與活動選用</small></div>
        <div className="metric"><span>離職／停用</span><strong>{visibleRows.filter((row) => row.employmentStatus === "INACTIVE").length}</strong><small>保留歷史，不供新流程選用</small></div>
      ...
      <div className="management-catalog-result"><p className="muted" role="status">{displayMessage}；符合條件 {sortedRows.length} 筆</p>{query || status !== "ALL" || institutionCode ? <button className="text-button product-filter-reset" type="button" onClick={() => { setQuery(""); setStatus("ALL"); setInstitutionCode(""); setPage(1); }}>清除篩選</button> : null}</div>
